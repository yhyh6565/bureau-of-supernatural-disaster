...
### 페르소나 키 로그인
- **로그인 방식**: ID에 한글 이름 입력 (비밀번호는 형식적 절차)
...
|------|------|-----|------|--------|------|------|-------------|
| **박홍림** | 출동구조반 | 현무1팀 | HMU-001 | 홍화 | 6급 | 팀장, 리더십 | 쪽지 1건 |
| **최요원** | 출동구조반 | 현무1팀 | HMU-002 | 미상 | 7급 | 사고 다발, 주간 검진 | 재난 3건, 쪽지 4건, 공지 2건, 결재 3건, 일정 5건 |
| **류재관** | 출동구조반 | 현무1팀 | HMU-003 | 청동 | 8급 | 부적 전문, 체계적 | 재난 2건, 쪽지 3건, 공지 1건, 결재 3건, 일정 4건 |
| **김솔음** | 출동구조반 | 현무1팀 | HMU-004 | 포도 | 7급 | 주인공, 창의적 해결 | 재난 2건, 쪽지 4건, 공지 1건, 결재 2건, 일정 4건 |
| **해금** | 출동구조반 | 현무3팀 | HMU-301 | 해금 | 6급 | 3팀 팀장, 멸형급 대응 | 재난 2건, 쪽지 2건, 공지 1건, 결재 2건, 일정 3건 |
| **고영은** | 신규조사반 | 백호2팀 | BKH-201 | 박하 | 7급 | 의료 지식, 차분함 | 재난 2건, 쪽지 3건, 공지 1건, 결재 2건, 일정 3건 |
| **장허운** | 현장정리반 | 주작2팀 | JJK-201 | 화각 | 7급 | 현장정리, 심리 지원 필요 | 재난 2건, 쪽지 3건, 공지 1건, 결재 2건, 일정 3건 |
...
| `/visits` | VisitsPage | 방문 신청 | 시설 예약 (도깨비 공방, 바리데기 세공소 등) |
| `/equipment` | EquipmentPage | 장비/서비스 | 장비 대여/지급, 기숙사 신청 |
...
|------|------|------|
| **GNB** | 상단 | 7대 메뉴 네비게이션 |
| **주 단위 일정** | 좌측 상단 | 오늘(D-day) ±3일 (총 7일) 타임라인 뷰 |
| **리스트** | 좌측 하단 | 소속별 핵심 업무 목록 (부서마다 다름) |
| **현황** | 우측 상단 | 부서별 핵심 KPI 통계 (접수/완료 건수 등) |
| **배정** | 우측 하단 | '나(또는 팀)'에게 할당된 구체적 업무 |
...
**대시보드 구성**:
- **리스트** (Read-only): 외부(경찰/민간)에서 접수된 조사 요청 목록 (개인 선택 불가)
...
**대시보드 구성**:
- **리스트** (2개 탭):
  - **긴급 구조**: 실시간 발생 요청 (클릭 시 [승낙 팝업(서명)] 후 배정)
  - **정기 순찰**: 오늘 예정된 순찰 구역 (클릭 시 [순찰 임무 시작] 후 배정)
- **현황**: 월별 요청 / 구조 중 / 완료 건수
- **배정**: 우리 팀이 맡은 긴급/순찰 임무 목록
...
**대시보드 구성**:
1. **[좌측 하단] 리스트** (정리 요청): 현무팀(구조반)이나 상부에서 넘어온 실시간 정리 대기 목록 (Read Only)
2. **[우측 상단] 현황** (정리 통계): 이번 달 요청 / 진행 중 / 정리 완료 건수
3. **[우측 하단] 배정** (나의 업무): 팀장이 나에게 배정한 정리 건

**담당 업무 페이지**:
1. **배정된 정리 건 리스트**: 본인 할당 목록
2. **상세 정보 뷰어**:
...
- **상세 속성 정보**:
  ...
  - 분석 데이터: 어둠의 종류, 파훼법, 진입 금지 조건 (조사 후 업데이트)
...
│   ├── extendedMockData.ts # 공통 타입 정의 (Message, ApprovalDocument)
│   ├── global/           # 전사 공통 데이터 (모든 사용자 동일)
│   │   ├── incidents.ts     # 기본 재난 5건
│   │   ├── notifications.ts # 전사 공지사항
│   │   ├── equipment.ts     # 장비 목록 및 재고
│   │   └── locations.ts     # 방문 가능 시설
│   ├── ordinary/         # 평범한 요원 개인 데이터
│   │   ├── incidents.ts     # 추가 재난 (현재 비어있음)
│   │   ├── messages.ts      # 일상 쪽지
│   │   ├── approvals.ts     # 일반 결재 문서
│   │   └── schedules.ts     # 일반 일정
│   └── personas/         # 7명 페르소나별 개인 데이터
...
© 2025 환경부 초자연재난관리국 | v1.2-CLASSIFIED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
