...
### 퇴사자 기능 차단 정책 (Resigned Agent Restrictions) [NEW]
보안 규정에 따라 **퇴사(Resigned)** 상태인 요원은 시스템 접속은 가능하나, 쓰기/수정 권한이 박탈됩니다.

- **차단 대상**: 상태값이 `resigned`인 요원 (예: 고영은)
- **접속 권한**: 로그인 및 Read-Only 접근 가능 (기존 데이터 열람용)
- **기능 제한**: 다음 액션 수행 시 거부 메시지 출력
  1. **장비 대여 및 지급 신청**
  2. **시설 방문 예약**
  3. **오염 검사 신청**
  4. **신규 업무 승낙**
  5. **매뉴얼 댓글 작성**
  6. **쪽지 발송**
- **UI UX**: 버튼은 활성화되어 있으나 클릭 시 **"퇴사 처리된 요원은 이용할 수 없습니다."** 토스트 메시지와 함께 차단됨.
- **참고**: `휴직(Leave)` 상태 요원은 해당 제한이 적용되지 않습니다.
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| **v2.3** | 2026-01-14 | **시스템 구조 개선**: `WorkContext`를 4개의 모듈식 훅(`useWorkIncidents`, `useWorkSchedules` 등)으로 분리하여 유지보수성 향상, **데이터 정합성 강화**: CSV 데이터 빌드 시 숫자 필드(`contamination`, `grade`) 자동 형변환 적용 |
...
- **발동 조건**: 로그인 후 30초 경과 (`time-elapsed`)
- **이벤트 시퀀스**:
  ...
  4. **긴급 알림 팝업**: 화면 우측 하단에 '긴급 공지 발생' 토스트 팝업이 5초간 표시됨
  5. **재난 목록 갱신**: 재난 현황 페이지에 '강남역 싱크홀' 건이 페이드인 효과와 함께 등장 (반짝임 효과 3회)
...
### 8.6 특수 상호작용 및 이스터에그 (Interaction & Easter Eggs) - [NEW v2.2.1]

시스템의 몰입감을 높이기 위해 특정 조건에서 발동하는 숨겨진 기능들이 존재합니다.

#### 1) 공포 메시지 (Haunted Message)
*   **대상**: 일반 유저 (랜덤 생성된 요원)
*   **발동 조건**: 로그인 후 **1분** 경과 시
*   **효과**:
    *   **쪽지 수신**: 발신자 '■■■(현장탐사반)', 제목 '오지마', 내용 '지사로 오지마... (반복)'
    *   **UI 변조**: 수신일이 **"20■■.05.04"**라는 깨진 텍스트로 강제 표시됨 (실제 데이터는 과거 날짜로 정렬 로직상 최하단 배치).
    *   **토스트 알림**: 화면 우측 하단에 검은색 배경의 "■■■ 쪽지가 도착했습니다" 알림 발생.
