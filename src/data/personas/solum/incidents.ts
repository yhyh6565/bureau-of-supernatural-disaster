import { Incident } from '@/types/haetae';

export const SOLUM_INCIDENTS: Incident[] = [
    {
        id: 'inc-006',
        caseNumber: '20251222-003',
        registrationNumber: '0006PSYA.2025.가06',
        location: '서울특별시 종로구 북촌 한옥',
        dangerLevel: '뇌형',
        status: '구조중',
        reportContent: '한옥 내부 시공간 왜곡. 포도 요원 임기응변으로 2명 구조.',
        requiresPatrol: false,
        countermeasure: '메모리얼 그립톡 활용',
        createdAt: new Date('2025-12-22T15:00:00'),
        updatedAt: new Date('2025-12-23T09:00:00'),
    },
    {
        id: 'inc-007',
        caseNumber: '20251210-001',
        registrationNumber: '0007PSYA.2025.가07',
        location: '부산광역시 해운대구 해변',
        dangerLevel: '고형',
        status: '종결',
        reportContent: '해변 모래성에서 이상 현상. 포도 요원 창의적 대응으로 신속 해결.',
        requiresPatrol: false,
        countermeasure: '소금 결계',
        createdAt: new Date('2025-12-10T11:00:00'),
        updatedAt: new Date('2025-12-11T14:00:00'),
    },
];
