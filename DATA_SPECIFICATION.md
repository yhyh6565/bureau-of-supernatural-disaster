...
**변경 이력**:
- **v2.3 (2026-01-14)**: CSV 빌드 스크립트(`build-data.ts`) 개선 - `contamination`, `grade`, `totalStock` 등 숫자 필드 자동 형변환 적용
- **v2.2 (2026-01-04)**: 박홍림 오염도 예외 규칙, 결재 필터링 로직 개선, 자동 데이터 보정(`processedAt`)
- **v2.1 (2026-01-04)**: 류재관 장비 추가, 공지사항 핀 기능 추가
- **v2.0 (2026-01-03)**: 재난 상태 판단 로직 중앙화(`processedIncidents`) 및 세션 기반 상태 오버라이드 규칙 추가
- **v1.9 (2026-01-03)**: 정신 오염도 시스템(자동 증가, 게임오버) 및 이스터에그 상세 명세 추가
...
- **자동 보정**: 데이터 생성 시 스크립트(`generateDynamicData.cjs`)가 `status: "승인"`인 문서의 `processedAt`을 `createdAt`과 동일하게 자동 설정합니다.
...
### 3.11 Persona Profile (페르소나 프로필)
*   **파일**: `personas/{name}/profile.json`
*   **설명**: 해당 페르소나의 신상 정보 및 특수 규칙
*   **주요 규칙**:
    *   **장허운**: `contamination`이 **95%**로 고정되며, 자동 증가 로직(`UserContext`)에서 제외됩니다.
    *   **최요원**: `codename`이 **'■■'**로 표시됩니다. 초기 오염도는 22%입니다.
    *   **박홍림**: `contamination`이 **98%**로 고정되며, 게임오버가 발생하지 않습니다.
    *   **기본 설정**: 현무 부서 요원은 로그인 시 자동으로 `rent-shoelace` (오방색 신발끈) 아이템이 지급됩니다.
...
// 내가 작성한 결재 문서 필터링 (v2.2 개선: 상태 무관 모든 내 문서 포함)
const myApprovals = approvals.filter(a =>
  a.createdBy === agent?.personaKey || a.createdBy === agent?.id
);

// 결재 대기/완료함 필터링 (v2.2 개선: 내가 결재할 문서 + 내가 기안한 대기/완료 문서 통합)
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| v2.1 | 2026-01-04 | 류재관 요원 보유 장비 추가, 공지사항 스키마 업데이트(`isPinned`), '지산소' 사건 위치 수정 |
| v2.0 | 2026-01-03 | 컴포넌트 리팩토링 (`ResourcesPage` 분리, `dateUtils.ts` 통합), Dashboard 데이터 소스 통일 (`useWork` 강제) |
| v1.9 | 2026-01-03 | 정신 오염도 시스템(자동 증가, 게임오버) 및 이스터에그 상세 명세 추가 |
| v1.8 | 2026-01-03 | 상호작용 데이터 통합, 트리거(`trigger`) 필드 추가, 검사 예약(`inspections.json`) 분리, 재난 데이터 갱신 |
...
### 3.7 보안 및 금지된 신원 (Security & Forbidden Personas)
- **금지된 신원 목록 (Forbidden Names)**: 시스템 접속 시도 시 즉시 '정신 오염 게임오버' 처분
  - 호유원, 청달래, 백석주, 진나솔, 이석종, 강도준, 이성해, 이자헌, 은하제, 박민성, 윤조훈
  - 백사헌, 강이학, 이재진, 이승조, 곽제강, 이연화, 최명진, 박경환, 이병진, 이강헌, 김허운
- **특수 인증 필요 신원**: 최요원, 해금 (긴급 인증 모달 발동)
